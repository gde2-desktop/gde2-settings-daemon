...
#include <libgde2-desktop/gde2-rr-config.h>
#include <libgde2-desktop/gde2-rr.h>
#include <libgde2-desktop/gde2-rr-labeler.h>
...
#define CONF_SCHEMA                                    "org.gde2.SettingsDaemon.plugins.xrandr"
...
#define MSD_XRANDR_DISPLAY_CAPPLET "gde2-display-properties"

#define MSD_DBUS_PATH "/org/gde2/SettingsDaemon"
#define MSD_DBUS_NAME "org.gde2.SettingsDaemon"
...
struct MsdXrandrManagerPrivate
{
        ...
        Gde2RRConfig **fn_f7_configs;  /* NULL terminated, NULL if there are no configs */

        /* Last time at which we got a "screen got reconfigured" event; see on_randr_event() */
        guint32 last_config_timestamp;
};

static const Gde2RRRotation possible_rotations[] = {
        GDE2_RR_ROTATION_0,
        GDE2_RR_ROTATION_90,
        GDE2_RR_ROTATION_180,
        GDE2_RR_ROTATION_270
        /* We don't allow REFLECT_X or REFLECT_Y for now, as gde2-display-properties doesn't allow them, either */
};
...
static void get_allowed_rotations_for_output (Gde2RRConfig *config,
                                              Gde2RRScreen *rr_screen,
                                              Gde2RROutputInfo *output,
                                              int *out_num_rotations,
                                              Gde2RRRotation *out_rotations);
...
static void
log_output (Gde2RROutputInfo *output)
{
        gchar *name = gde2_rr_output_info_get_name (output);
        gchar *display_name = gde2_rr_output_info_get_display_name (output);

        log_msg ("        %s: ", name ? name : "unknown");

        if (gde2_rr_output_info_is_connected (output)) {
                if (gde2_rr_output_info_is_active (output)) {
                        int x, y, width, height;
                        gde2_rr_output_info_get_geometry (output, &x, &y, &width, &height);
                        log_msg ("%dx%d@%d +%d+%d",
                                 width,
                                 height,
                                 gde2_rr_output_info_get_refresh_rate (output),
                                 x,
                                 y);
                } else
                        log_msg ("off");
        ...
        if (gde2_rr_output_info_get_primary (output))
...
static void
log_configuration (Gde2RRConfig *config)
{
        int i;
        Gde2RROutputInfo **outputs = gde2_rr_config_get_outputs (config);

        log_msg ("        cloned: %s\n", gde2_rr_config_get_clone (config) ? "yes" : "no");
...
static void
log_screen (Gde2RRScreen *screen)
{
        Gde2RRConfig *config;
        ...
        config = gde2_rr_config_new_current (screen, NULL);

        gde2_rr_screen_get_ranges (screen, &min_w, &max_w, &min_h, &max_h);
        gde2_rr_screen_get_timestamps (screen, &change_timestamp, &config_timestamp);
...
static void
log_configurations (Gde2RRConfig **configs)
{
        int i;
...
show_timestamps_dialog (MsdXrandrManager *manager, const char *msg)
{
        ...
        gde2_rr_screen_get_timestamps (priv->rw_screen, &change_timestamp, &config_timestamp);
...
/* This function centralizes the use of gde2_rr_config_apply_from_filename_with_time().
 *
 * Optionally filters out GDE2_RR_ERROR_NO_MATCHING_CONFIG from
 * gde2_rr_config_apply_from_filename_with_time(), since that is not usually an error.
 */
static gboolean
                                   ...
                                   GError          **error)
{
        ...
        my_error = NULL;
        success = gde2_rr_config_apply_from_filename_with_time (priv->rw_screen, filename, timestamp, &my_error);
        if (success)
                return TRUE;

        if (g_error_matches (my_error, GDE2_RR_ERROR, GDE2_RR_ERROR_NO_MATCHING_CONFIG)) {
...
/* This function centralizes the use of gde2_rr_config_apply_with_time().
 *
 ...
 */
static gboolean
apply_configuration_and_display_error (MsdXrandrManager *manager, Gde2RRConfig *config, guint32 timestamp)
{
        ...
        error = NULL;
        success = gde2_rr_config_apply_with_time (config, priv->rw_screen, timestamp, &error);
        if (!success) {
...
{
        backup_filename = gde2_rr_config_get_backup_filename ();
...
confirm_with_user_idle_cb (gpointer data)
{
        ...
        backup_filename = gde2_rr_config_get_backup_filename ();
        intended_filename = gde2_rr_config_get_intended_filename ();
...
/* DBus method for org.gde2.SettingsDaemon.XRANDR ApplyConfiguration; see msd-xrandr-manager.xml for the interface definition */
static gboolean
...
/* DBus method for org.gde2.SettingsDaemon.XRANDR_2 ApplyConfiguration; see msd-xrandr-manager.xml for the interface definition */
static gboolean
...
static gboolean
is_laptop (Gde2RRScreen *screen, Gde2RROutputInfo *output)
{
        Gde2RROutput *rr_output;

        rr_output = gde2_rr_screen_get_output_by_name (screen, gde2_rr_output_info_get_name (output));
        return gde2_rr_output_is_laptop (rr_output);
}

static gboolean
get_clone_size (Gde2RRScreen *screen, int *width, int *height)
{
        Gde2RRMode **modes = gde2_rr_screen_list_clone_modes (screen);
        int best_w, best_h;
        int i;
        ...
        for (i = 0; modes[i] != NULL; ++i) {
                Gde2RRMode *mode = modes[i];
                int w, h;

                w = gde2_rr_mode_get_width (mode);
                h = gde2_rr_mode_get_height (mode);
...
static void
print_output (Gde2RROutputInfo *info)
{
        int x, y, width, height;

        g_print ("  Output: %s attached to %s\n", gde2_rr_output_info_get_display_name (info), gde2_rr_output_info_get_name (info));
        g_print ("     status: %s\n", gde2_rr_output_info_is_active (info) ? "on" : "off");

        gde2_rr_output_info_get_geometry (info, &x, &y, &width, &height);
        g_print ("     width: %d\n", width);
        g_print ("     height: %d\n", height);
        g_print ("     rate: %d\n", gde2_rr_output_info_get_refresh_rate (info));
        g_print ("     position: %d %d\n", x, y);
}

static void
print_configuration (Gde2RRConfig *config, const char *header)
{
        int i;
        ...
        outputs = gde2_rr_config_get_outputs (config);
        for (i = 0; outputs[i] != NULL; ++i)
                print_output (outputs[i]);
}

static gboolean
config_is_all_off (Gde2RRConfig *config)
{
        int j;
        Gde2RROutputInfo **outputs;
        ...
        for (j = 0; outputs[j] != NULL; ++j) {
                if (gde2_rr_output_info_is_active (outputs[j])) {
                        return FALSE;
                }
        }
...
static Gde2RRConfig *
make_clone_setup (Gde2RRScreen *screen)
{
        Gde2RRConfig *result;
        Gde2RROutputInfo **outputs;
        int width, height;
        ...
        result = gde2_rr_config_new_current (screen, NULL);
        outputs = gde2_rr_config_get_outputs (result);

        for (i = 0; outputs[i] != NULL; ++i) {
                Gde2RROutputInfo *info = outputs[i];

                gde2_rr_output_info_set_active (info, FALSE);
                if (gde2_rr_output_info_is_connected (info)) {
                        Gde2RROutput *output =
                                gde2_rr_screen_get_output_by_name (screen, gde2_rr_output_info_get_name (info));
                        ...
                        for (j = 0; modes[j] != NULL; ++j) {
                                ...
                                if (w == width && h == height) {
                                        int r = gde2_rr_mode_get_freq (mode);
                                        if (r > best_rate)
                                                best_rate = r;
                                }
                        }
                        ...
                        if (best_rate > 0) {
                                gde2_rr_output_info_set_active (info, TRUE);
                                gde2_rr_output_info_set_rotation (info, GDE2_RR_ROTATION_0);
                                gde2_rr_output_info_set_refresh_rate (info, best_rate);
                                gde2_rr_output_info_set_geometry (info, 0, 0, width, height);
                        }
                }
        }
...
static Gde2RRMode *
find_best_mode (Gde2RROutput *output)
{
        Gde2RRMode *preferred;
        Gde2RRMode **modes;
        int best_size;
        int best_width, best_height, best_rate;
        int i;
        Gde2RRMode *best_mode;

        preferred = gde2_rr_output_get_preferred_mode (output);
        if (preferred)
                return preferred;

        modes = gde2_rr_output_list_modes (output);
        if (!modes)
                return NULL;
        ...
        for (i = 0; modes[i] != NULL; i++) {
                ...
                w = gde2_rr_mode_get_width (modes[i]);
                h = gde2_rr_mode_get_height (modes[i]);
                r = gde2_rr_mode_get_freq  (modes[i]);
...
static gboolean
turn_on (Gde2RRScreen *screen,
         Gde2RROutputInfo *info,
         int x, int y)
{
        Gde2RROutput *output = gde2_rr_screen_get_output_by_name (screen, gde2_rr_output_info_get_name (info));
        Gde2RRMode *mode = find_best_mode (output);

        if (mode) {
                gde2_rr_output_info_set_active (info, TRUE);
                gde2_rr_output_info_set_geometry (info, x, y, gde2_rr_mode_get_width (mode), gde2_rr_mode_get_height (mode));
                gde2_rr_output_info_set_rotation (info, GDE2_RR_ROTATION_0);
                gde2_rr_output_info_set_refresh_rate (info, gde2_rr_mode_get_freq (mode));
...
static Gde2RRConfig *
make_laptop_setup (Gde2RRScreen *screen)
{
        /* Turn on the laptop, disable everything else */
        Gde2RRConfig *result = gde2_rr_config_new_current (screen, NULL);
        Gde2RROutputInfo **outputs = gde2_rr_config_get_outputs (result);
        int i;
...
static int
turn_on_and_get_rightmost_offset (Gde2RRScreen *screen, Gde2RROutputInfo *info, int x)
{
        if (turn_on (screen, info, x, 0)) {
                int width;
                gde2_rr_output_info_get_geometry (info, NULL, NULL, &width, NULL);
                x += width;
        }
...
static Gde2RRConfig *
make_xinerama_setup (Gde2RRScreen *screen)
{
	...
	Gde2RROutputInfo **outputs = gde2_rr_config_get_outputs (result);
        ...
        for (i = 0; outputs[i] != NULL; ++i) {
                ...
                if (gde2_rr_output_info_is_connected (info) && !is_laptop (screen, info))
                        x = turn_on_and_get_rightmost_offset (screen, info, x);
        }
...
static Gde2RRConfig *
make_other_setup (Gde2RRScreen *screen)
{
...
sanitize (MsdXrandrManager *manager, GPtrArray *array)
{
        ...
        for (i = 0; i < array->len; i++) {
                ...
                for (j = i + 1; j < array->len; j++) {
                        Gde2RRConfig *this = array->pdata[j];
                        Gde2RRConfig *other = array->pdata[i];

                        if (this && other && gde2_rr_config_equal (this, other)) {
        ...
        for (i = 0; i < array->len; ++i) {
                Gde2RRConfig *config = array->pdata[i];
        ...
        for (i = 0; i < array->len; i++) {
                ...
                if (config) {
                        ...
                        error = NULL;
                        if (!gde2_rr_config_applicable (config, manager->priv->rw_screen, &error)) { /* NULL-GError */
...
generate_fn_f7_configs (MsdXrandrManager *mgr)
{
        ...
        Gde2RRScreen *screen = mgr->priv->rw_screen;
        ...
        g_ptr_array_add (array, gde2_rr_config_new_current (screen, NULL));
        ...
        if (array) {
                mgr->priv->fn_f7_configs = (Gde2RRConfig **)g_ptr_array_free (array, FALSE);
...
{
        MsdXrandrManagerPrivate *priv = mgr->priv;
        Gde2RRScreen *screen = priv->rw_screen;
         ...
         * a list of Gde2RRConfig's. Each of the Gde2RRConfigs has a
         * mode (or "off") for each connected output.
         *
         * When the user hits fn-F7, we cycle to the next Gde2RRConfig
        ...
        error = NULL;
        if (!gde2_rr_screen_refresh (screen, &error) && error) {
                char *str;
        ...
        current = gde2_rr_config_new_current (screen, NULL);

        if (priv->fn_f7_configs &&
            (!gde2_rr_config_match (current, priv->fn_f7_configs[0]) ||
             !gde2_rr_config_equal (current, priv->fn_f7_configs[mgr->priv->current_fn_f7_config]))) {
        ...
        if (priv->fn_f7_configs) {
                 ...
                 */
                gde2_rr_screen_get_timestamps (screen, NULL, &server_timestamp);
...
get_laptop_output_info (Gde2RRScreen *screen, Gde2RRConfig *config)
{
        int i;
        Gde2RROutputInfo **outputs = gde2_rr_config_get_outputs (config);
...
get_next_rotation (Gde2RRRotation allowed_rotations, Gde2RRRotation current_rotation)
{
        int i;
        int current_index;
...
handle_rotate_windows (MsdXrandrManager *mgr, guint32 timestamp)
{
        ...
        Gde2RRScreen *screen = priv->rw_screen;
        Gde2RRConfig *current;
        Gde2RROutputInfo *rotatable_output_info;
        int num_allowed_rotations;
        Gde2RRRotation allowed_rotations;
        Gde2RRRotation next_rotation;
        ...
        next_rotation = get_next_rotation (allowed_rotations, gde2_rr_output_info_get_rotation (rotatable_output_info));

        if (next_rotation == gde2_rr_output_info_get_rotation (rotatable_output_info)) {
        ...
        gde2_rr_output_info_set_rotation (rotatable_output_info, next_rotation);
...
auto_configure_outputs (MsdXrandrManager *manager, guint32 timestamp)
{
        ...
        config = gde2_rr_config_new_current (priv->rw_screen, NULL);
        ...
        for (i = 0; outputs[i] != NULL; i++) {
                Gde2RROutputInfo *output = outputs[i];

                if (gde2_rr_output_info_is_connected (output) && !gde2_rr_output_info_is_active (output)) {
                        gde2_rr_output_info_set_active (output, TRUE);
                        gde2_rr_output_info_set_rotation (output, GDE2_RR_ROTATION_0);
                        just_turned_on = g_list_prepend (just_turned_on, GINT_TO_POINTER (i));
                } else if (!gde2_rr_output_info_is_connected (output) && gde2_rr_output_info_is_active (output))
                        gde2_rr_output_info_set_active (output, FALSE);
        }
        ...
        for (i = 0; outputs[i] != NULL; i++) {
                ...
                if (gde2_rr_output_info_is_active (output)) {
                        int width, height;
                        g_assert (gde2_rr_output_info_is_connected (output));

                        gde2_rr_output_info_get_geometry (output, NULL, NULL, &width, &height);
			gde2_rr_output_info_set_geometry (output, x, 0, width, height);
		...
		int width;
                ...
                g_assert (gde2_rr_output_info_is_active (output) && gde2_rr_output_info_is_connected (output));

                /* since the output was off, use its preferred width/height (it doesn't have a real width/height yet) */
                width = gde2_rr_output_info_get_preferred_width (output);
                gde2_rr_output_info_set_geometry (output, x, 0, width, gde2_rr_output_info_get_preferred_height (output));
        ...
        while (1) {
                ...
                error = NULL;
                applicable = gde2_rr_config_applicable (config, priv->rw_screen, &error);

                if (applicable)
                        break;

                is_bounds_error = g_error_matches (error, GDE2_RR_ERROR, GDE2_RR_ERROR_BOUNDS_ERROR);
                g_error_free (error);
...
apply_color_profiles (void)
{
        ...
        /* run the gde2-color-manager apply program */
...
static void
on_randr_event (Gde2RRScreen *screen, gpointer data)
{
        ...
        if (change_timestamp >= config_timestamp) {
                ...
                if (!success) {
                         ...
                         * GDE2_RR_ERROR_NO_MATCHING_CONFIG would mean, "there
...
run_display_capplet (GtkWidget *widget)
{
        ...
        error = NULL;
        if (!gde2_gdk_spawn_command_line_on_screen (screen, MSD_XRANDR_DISPLAY_CAPPLET, &error)) {
                GtkWidget *dialog;
...
status_icon_popup_menu_selection_done_cb (GtkMenuShell *menu_shell, gpointer data)
{
        ...
        gde2_rr_labeler_hide (priv->labeler);
...
/* This is an expose-event hander for the title label for each Gde2RROutput.
...
#if GTK_CHECK_VERSION (3, 0, 0)
        gde2_rr_labeler_get_rgba_for_output (priv->labeler, output, &color);
#else
        gde2_rr_labeler_get_color_for_output (priv->labeler, output, &color);
...
static GtkWidget *
make_menu_item_for_output_title (MsdXrandrManager *manager, Gde2RROutputInfo *output)
{
        GtkWidget *item;
        GtkWidget *label;
        char *str;
        ...
        str = g_markup_printf_escaped ("<b>%s</b>", gde2_rr_output_info_get_display_name (output));
...
static void
get_allowed_rotations_for_output (Gde2RRConfig *config,
                                  Gde2RRScreen *rr_screen,
                                  Gde2RROutputInfo *output,
                                  int *out_num_rotations,
                                  Gde2RRRotation *out_rotations)
{
        ...
        *out_rotations = 0;
        ...
        current_rotation = gde2_rr_output_info_get_rotation (output);

        /* Yay for brute force */

        for (i = 0; i < G_N_ELEMENTS (possible_rotations); i++) {
                Gde2RRRotation rotation_to_test;

                rotation_to_test = possible_rotations[i];

                gde2_rr_output_info_set_rotation (output, rotation_to_test);

                if (gde2_rr_config_applicable (config, rr_screen, NULL)) { /* NULL-GError */
                        (*out_num_rotations)++;
                        (*out_rotations) |= rotation_to_test;
                }
        }

        gde2_rr_output_info_set_rotation (output, current_rotation);
        ...
        if (*out_num_rotations == 0 || *out_rotations == 0) {
                ...
                *out_num_rotations = 1;
                *out_rotations = gde2_rr_output_info_get_rotation (output);
        }
}
...
{
        Gde2RRScreen *rr_screen;
        Gde2RRConfig *rr_config;

        /* Normally, gde2_rr_config_save() creates a backup file based on the
         * old monitors.xml.  However, if *that* file didn't exist, there is
         * nothing from which to create a backup.  So, here we'll save the
         * current/unchanged configuration and then let our caller call
         * gde2_rr_config_save() again with the new/changed configuration, so
         * that there *will* be a backup file in the end.
         */

        rr_screen = gde2_rr_screen_new (gdk_screen_get_default (), NULL); /* NULL-GError */
        if (!rr_screen)
                return;

        rr_config = gde2_rr_config_new_current (rr_screen, NULL);
        gde2_rr_config_save (rr_config, NULL); /* NULL-GError */
...
output_rotation_item_activate_cb (GtkCheckMenuItem *item, gpointer data)
{
        ...
        gde2_rr_output_info_set_rotation (output, rotation);

        error = NULL;
        if (!gde2_rr_config_save (priv->configuration, &error)) {
...
static void
add_items_for_rotations (MsdXrandrManager *manager, Gde2RROutputInfo *output, Gde2RRRotation allowed_rotations)
{
        typedef struct {
                Gde2RRRotation	rotation;
                const char *	name;
        ...
        static const RotationInfo rotations[] = {
                { GDE2_RR_ROTATION_0, N_("Normal") },
                { GDE2_RR_ROTATION_90, N_("Left") },
                { GDE2_RR_ROTATION_270, N_("Right") },
                { GDE2_RR_ROTATION_180, N_("Upside Down") },
                /* We don't allow REFLECT_X or REFLECT_Y for now, as gde2-display-properties doesn't allow them, either */
        };
        ...
        for (i = 0; i < G_N_ELEMENTS (rotations); i++) {
                ...
                if (rot == gde2_rr_output_info_get_rotation (output)) {
                        active_item = item;
...
static void
add_rotation_items_for_output (MsdXrandrManager *manager, Gde2RROutputInfo *output)
{
        struct MsdXrandrManagerPrivate *priv = manager->priv;
        int num_rotations;
...
static void
add_menu_items_for_output (MsdXrandrManager *manager, Gde2RROutputInfo *output)
{
        struct MsdXrandrManagerPrivate *priv = manager->priv;
        GtkWidget *item;
...
add_menu_items_for_outputs (MsdXrandrManager *manager)
{
        ...
        outputs = gde2_rr_config_get_outputs (priv->configuration);
        for (i = 0; outputs[i] != NULL; i++) {
                if (gde2_rr_output_info_is_connected (outputs[i]))
...
status_icon_popup_menu (MsdXrandrManager *manager, guint button, guint32 timestamp)
{
        ...
        priv->configuration = gde2_rr_config_new_current (priv->rw_screen, NULL);

        g_assert (priv->labeler == NULL);
        priv->labeler = gde2_rr_labeler_new (priv->configuration);
        ...
        /*Set up the gtk theme class from gde2-panel*/
        ...
        if (!result) {
                if (my_error) {
                        if (!g_error_matches (my_error, G_FILE_ERROR, G_FILE_ERROR_NOENT) &&
                            !g_error_matches (my_error, GDE2_RR_ERROR, GDE2_RR_ERROR_NO_MATCHING_CONFIG))
                          ...
                          GError          **error)
{
        ...
        manager->priv->rw_screen = gde2_rr_screen_new (gdk_screen_get_default (), error);
