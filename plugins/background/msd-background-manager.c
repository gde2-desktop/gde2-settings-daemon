...
#define gde2_bg_set_surface_as_root_with_crossfade	gde2_bg_set_pixmap_as_root_with_crossfade
#endif

#define GDE2_SESSION_MANAGER_DBUS_NAME "org.gnome.SessionManager"
#define GDE2_SESSION_MANAGER_DBUS_PATH "/org/gnome/SessionManager"
...
{
	return g_settings_get_boolean (manager->priv->settings, GDE2_BG_KEY_DRAW_BACKGROUND);
}
...
{
	return g_settings_get_boolean (manager->priv->settings, GDE2_BG_KEY_BACKGROUND_FADE);
}
...
{
	return g_settings_get_boolean (manager->priv->settings, GDE2_BG_KEY_SHOW_DESKTOP);
}
	      ...
	      GdkScreen		   *screen)
{
	...
	p->surface = gde2_bg_create_surface (p->bg, window, width, height, TRUE);

	if (p->do_fade)
	{
		free_fade (manager);
		p->fade = gde2_bg_set_surface_as_root_with_crossfade (screen, p->surface);
		g_signal_connect_swapped (p->fade, "finished", G_CALLBACK (free_fade), manager);
	}
	else
	{
		gde2_bg_set_surface_as_root (screen, p->surface);
	}
...
{
	gde2_settings_profile_start ("settings_change_event_idle_cb");

	gde2_bg_load_from_gsettings (manager->priv->bg,
				     manager->priv->settings);

	gde2_settings_profile_end ("settings_change_event_idle_cb");
...
setup_background (MsdBackgroundManager *manager)
{
	...
	gde2_bg_load_from_gsettings (p->bg, p->settings);
...
draw_bg_after_session_loads (MsdBackgroundManager *manager)
{
	...
	manager->priv->proxy = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SESSION,
							      ...
							      GDE2_SESSION_MANAGER_DBUS_NAME,
							      GDE2_SESSION_MANAGER_DBUS_PATH,
							      GDE2_SESSION_MANAGER_DBUS_NAME,
							      NULL, /* GCancellable */
							      &error);
			      ...
			      GError               **error)
{
	...
	p->settings = g_settings_new (GDE2_BG_SCHEMA);

	p->msd_can_draw = msd_can_draw_bg (manager);
	p->caja_can_draw = caja_can_draw_bg (manager);

	g_signal_connect (p->settings, "changed::" GDE2_BG_KEY_DRAW_BACKGROUND,
			  G_CALLBACK (on_bg_handling_changed), manager);
	g_signal_connect (p->settings, "changed::" GDE2_BG_KEY_SHOW_DESKTOP,
			  G_CALLBACK (on_bg_handling_changed), manager);
