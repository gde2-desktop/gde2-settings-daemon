...
#define GDE2KBD_DESKTOP_SCHEMA "org.gde2.peripherals-keyboard-xkb.general"
#define GDE2KBD_KBD_SCHEMA "org.gde2.peripherals-keyboard-xkb.kbd"
...
static Gde2kbdDesktopConfig current_desktop_config;
static Gde2kbdKeyboardConfig current_kbd_config;

/* never terminated */
static Gde2kbdKeyboardConfig initial_sys_kbd_config;
...
activation_error (void)
{
	...
	dialog = gtk_message_dialog_new_with_markup (NULL,
						     ...
						     release,
						     "xprop -root | grep XKB",
						     "gsettings list-keys org.gde2.peripherals-keyboard-xkb.kbd");
...
apply_desktop_settings (void)
{
	...
	gde2kbd_desktop_config_load_from_gsettings (&current_desktop_config);
	/* again, probably it would be nice to compare things
	   before activating them */
	gde2kbd_desktop_config_activate (&current_desktop_config);
...
popup_menu_launch_capplet ()
{
	...
	info = g_app_info_create_from_commandline ("gde2-keyboard-properties", NULL, 0, &error);
...
popup_menu_show_layout ()
{
	...
	gpointer p = g_hash_table_lookup (preview_dialogs,
					  GINT_TO_POINTER
					  (xkl_state->group));
	gchar **group_names = gde2kbd_status_get_group_names ();
	...
	dialog =
	    gde2kbd_keyboard_drawing_new_dialog (xkl_state->group,
					      group_names
					      [xkl_state->group]);
#  if GTK_CHECK_VERSION(3,0,0)
...
popup_menu_set_group (GtkMenuItem * item, gpointer param)
{
	...
	XklEngine *engine = gde2kbd_status_get_xkl_engine ();
	XklState st;
	Window cur;
...
status_icon_popup_menu_cb (GtkStatusIcon * icon, guint button, guint time)
{
	...
	int i = 0;
	gchar **current_name = gde2kbd_status_get_group_names ();
	...
	for (i = 0; *current_name; i++, current_name++) {
		gchar *image_file = gde2kbd_status_get_image_filename (i);
...
static gboolean
try_activating_xkb_config_if_new (Gde2kbdKeyboardConfig *
				  current_sys_kbd_config)
{
	/* Activate - only if different! */
	if (!gde2kbd_keyboard_config_equals
	    (&current_kbd_config, current_sys_kbd_config)) {
		if (gde2kbd_keyboard_config_activate (&current_kbd_config)) {
			if (pa_callback != NULL) {
...
filter_xkb_config (void)
{
	...
	if (!xkl_registry) {
		...
		/* load all gde2rials, unconditionally! */
	...
	while (*lv) {
		...
		if (gde2kbd_keyboard_config_split_items (*lv, &lname, &vname)) {
...
{
	Gde2kbdKeyboardConfig current_sys_kbd_config;

	if (!inited_ok)
		return;

	gde2kbd_keyboard_config_init (&current_sys_kbd_config, xkl_engine);

	gde2kbd_keyboard_config_load_from_gsettings (&current_kbd_config,
					      &initial_sys_kbd_config);

	gde2kbd_keyboard_config_load_from_x_current (&current_sys_kbd_config,
						  NULL);
	...
	gde2kbd_keyboard_config_term (&current_sys_kbd_config);
	show_hide_icon ();
}
...
msd_keyboard_xkb_analyze_sysconfig (void)
{
	...
	gde2kbd_keyboard_config_init (&initial_sys_kbd_config, xkl_engine);
	gde2kbd_keyboard_config_load_from_x_initial (&initial_sys_kbd_config,
						  NULL);
}
...
msd_keyboard_xkb_init (MsdKeyboardManager * kbd_manager)
{
	...
	manager = kbd_manager;
	gde2_settings_profile_start ("xkl_engine_get_instance");
	xkl_engine = xkl_engine_get_instance (display);
	gde2_settings_profile_end ("xkl_engine_get_instance");
	if (xkl_engine) {
		inited_ok = TRUE;

		settings_desktop = g_settings_new (GDE2KBD_DESKTOP_SCHEMA);
		settings_kbd = g_settings_new (GDE2KBD_KBD_SCHEMA);

		gde2kbd_desktop_config_init (&current_desktop_config,
		                             xkl_engine);
		gde2kbd_keyboard_config_init (&current_kbd_config,
		                              xkl_engine);

		xkl_engine_backup_names_prop (xkl_engine);
		msd_keyboard_xkb_analyze_sysconfig ();

		gde2kbd_desktop_config_start_listen (&current_desktop_config,
		                                     G_CALLBACK (apply_desktop_settings_cb),
		                                     NULL);

		gde2kbd_keyboard_config_start_listen (&current_kbd_config,
		...
		gde2_settings_profile_start ("xkl_engine_start_listen");
		xkl_engine_start_listen (xkl_engine,
					 XKLL_MANAGE_LAYOUTS |
					 XKLL_MANAGE_WINDOW_STATES);
		gde2_settings_profile_end ("xkl_engine_start_listen");

		gde2_settings_profile_start ("apply_desktop_settings");
		apply_desktop_settings ();
		gde2_settings_profile_end ("apply_desktop_settings");
		gde2_settings_profile_start ("apply_xkb_settings");
		apply_xkb_settings ();
		gde2_settings_profile_end ("apply_xkb_settings");
	}
