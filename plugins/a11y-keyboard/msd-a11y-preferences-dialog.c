...
#define KEY_A11Y_SCHEMA              "org.gde2.accessibility-keyboard"
...
#define KEY_AT_SCHEMA                   "org.gde2.applications-at"
...
#define KEY_INTERFACE_SCHEMA   "org.gde2.interface"
#define KEY_GTK_THEME          "gtk-theme"
#define KEY_COLOR_SCHEME       "gtk-color-scheme"
#define KEY_ICON_THEME         "icon-theme"

#define KEY_FINESTRA_SCHEMA    "org.gde2.Finestra"
#define KEY_FINESTRA_THEME     "theme"
...
config_set_high_contrast (MsdA11yPreferencesDialog *dialog, gboolean enabled)
{
        if (enabled) {
                ...
                /* there isn't a high contrast finestra theme afaik */
        } else {
                ...
                g_settings_reset (dialog->priv->settings_finestra, KEY_FINESTRA_THEME);
        }
}
              ...
              GtkBuilder               *builder)
{
        ...
        if (config_have_at_gsettings_condition ("GDE2 " KEY_AT_SCHEMA " " KEY_AT_SCREEN_KEYBOARD_ENABLED)) {
        ...
        gtk_widget_set_no_show_all (widget, TRUE);
        if (config_have_at_gsettings_condition ("GDE2 " KEY_AT_SCHEMA " " KEY_AT_SCREEN_READER_ENABLED)) {
                gtk_widget_show_all (widget);
        } else {
                gtk_widget_hide (widget);
        }
        ...
        gtk_widget_set_no_show_all (widget, TRUE);
        if (config_have_at_gsettings_condition ("GDE2 " KEY_AT_SCHEMA " " KEY_AT_SCREEN_MAGNIFIER_ENABLED)) {
                gtk_widget_show_all (widget);
        } else {
                gtk_widget_hide (widget);
        }
...
msd_a11y_preferences_dialog_init (MsdA11yPreferencesDialog *dialog)
{
        ...
        dialog->priv->settings_finestra = g_settings_new (KEY_FINESTRA_SCHEMA);
...
msd_a11y_preferences_dialog_finalize (GObject *object)
{
        ...
        g_object_unref (dialog->priv->settings_finestra);
