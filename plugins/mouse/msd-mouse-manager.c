...
#define GDE2_MOUSE_SCHEMA                "org.gde2.peripherals-mouse"
...
#define GDE2_TOUCHPAD_SCHEMA             "org.gde2.peripherals-touchpad"
...
/* FIXME: currently there is no gde2-mousetweaks, so I comment this
 *
 *#define GDE2_MOUSE_A11Y_SCHEMA         "org.gde2.accessibility-mouse"
...
set_xinput_devices_left_handed (MsdMouseManager * manager, gboolean left_handed)
{
        ...
        for (i = 0; i < n_devices; i++) {
                ...
                if (device != NULL) {
                        GSettings *settings = g_settings_new (GDE2_TOUCHPAD_SCHEMA);
...
{
        /* FIXME there is no gde2-mousetweaks */
        return;
        ...
        if (! g_spawn_command_line_async (comm, &error)) {
                if (error->code == G_SPAWN_ERROR_NOENT &&
                    (dwell_enable || delay_enable)) {
                        ...
                        /*
                        GSettings *settings;
                        settings = g_settings_new (GDE2_MOUSE_A11Y_SCHEMA);
                        if (dwell_enable)
                                g_settings_set_boolean (settings,
                                                        GDE2_MOUSE_A11Y_KEY_DWELL_ENABLE,
                                                        FALSE);
                        else if (delay_enable)
                                g_settings_set_boolean (settings,
                                                        GDE2_MOUSE_A11Y_KEY_DELAY_ENABLE,
                                                        FALSE);
                        g_object_unref (settings);
                        */
...
msd_mouse_manager_idle_cb (MsdMouseManager *manager)
{
        ...
        manager->priv->settings_mouse = g_settings_new (GDE2_MOUSE_SCHEMA);
        manager->priv->settings_touchpad = g_settings_new (GDE2_TOUCHPAD_SCHEMA);
