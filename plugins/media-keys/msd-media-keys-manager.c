...
#define TOUCHPAD_SCHEMA "org.gde2.peripherals-touchpad"
...
get_term_command (MsdMediaKeysManager *manager)
{
	...
	settings = g_settings_new ("org.gde2.applications-terminal");
	...
	if (cmd_term[0] != '\0') {
		...
		cmd = g_strdup_printf ("gde2-terminal -e");
	}
...
{
        execute (manager, "gde2-session-save --shutdown-dialog", FALSE, FALSE);
}

static void
do_logout_action (MsdMediaKeysManager *manager)
{
        execute (manager, "gde2-session-save --logout-dialog", FALSE, FALSE);
}
               ...
               gboolean             sound_changed)
{
        ...
        if (sound_changed != FALSE && muted == FALSE)
                ca_gtk_play_for_widget (manager->priv->dialog, 0,
                                        ...
                                        CA_PROP_APPLICATION_ID, "org.gde2.SettingsDaemon",
                                        NULL);
#endif
}
...
do_sound_action (MsdMediaKeysManager *manager, int type)
{
        ...
        volume_min = gde2_mixer_stream_control_get_min_volume (manager->priv->control);
        volume_max = gde2_mixer_stream_control_get_normal_volume (manager->priv->control);
        ...
        volume = volume_last =
                gde2_mixer_stream_control_get_volume (manager->priv->control);
        muted = muted_last =
                gde2_mixer_stream_control_get_mute (manager->priv->control);
        ...
        if (muted != muted_last) {
                if (gde2_mixer_stream_control_set_mute (manager->priv->control, muted))
                        sound_changed = TRUE;
                else
                        muted = muted_last;
        }
        if (volume != gde2_mixer_stream_control_get_volume (manager->priv->control)) {
                if (gde2_mixer_stream_control_set_volume (manager->priv->control, volume))
                        sound_changed = TRUE;
                else
                        volume = volume_last;
        }
...
update_default_output (MsdMediaKeysManager *manager)
{
        ...
        stream = gde2_mixer_context_get_default_output_stream (manager->priv->context);
        if (stream != NULL)
        ...
        if (control != NULL) {
                Gde2MixerStreamControlFlags flags = gde2_mixer_stream_control_get_flags (control);

                /* Do not use the stream if it is not possible to mute it or
                 * change the volume */
                if (!(flags & GDE2_MIXER_STREAM_CONTROL_MUTE_WRITABLE) &&
                    !(flags & GDE2_MIXER_STREAM_CONTROL_VOLUME_WRITABLE))
                        return;
                ...
                g_debug ("Default output stream updated to %s",
                         gde2_mixer_stream_get_name (stream));
        } else
...
static void
on_context_default_output_notify (Gde2MixerContext    *context,
                                  GParamSpec          *pspec,
                                  MsdMediaKeysManager *manager)
{
        update_default_output (manager);
                           ...
                           MsdMediaKeysManager *manager)
{
        if (manager->priv->stream != NULL) {
                Gde2MixerStream *stream =
                        gde2_mixer_context_get_stream (manager->priv->context, name);
	...
	case LOGOUT_KEY:
        ...
        case SEARCH_KEY:
                ...
                if ((cmd = g_find_program_in_path ("beagle-search"))) {
                        ...
                        execute (manager, "gde2-search-tool", FALSE, FALSE);
                }
                g_free (cmd);
                break;
        case EMAIL_KEY:
        ...
        case SCREENSAVER_KEY:
                if ((cmd = g_find_program_in_path ("gde2-screensaver-command"))) {
                        execute (manager, "gde2-screensaver-command --lock", FALSE, FALSE);
                } else {
        ...
        case CALCULATOR_KEY:
                ...
                } else if ((cmd = g_find_program_in_path ("gde2-calc"))) {
                        execute (manager, "gde2-calc", FALSE, FALSE);
                } else {
...
start_media_keys_idle_cb (MsdMediaKeysManager *manager)
{
        ...
        for (l = manager->priv->screens; l != NULL; l = l->next) {
                gde2_settings_profile_start ("gdk_window_add_filter");
                ...
                gdk_window_add_filter (gdk_screen_get_root_window (l->data),
                                       ...
                                       manager);
                gde2_settings_profile_end ("gdk_window_add_filter");
        }
...
#ifdef HAVE_LIBGDE2MIXER
        if (G_LIKELY (gde2_mixer_is_initialized ())) {
                gde2_settings_profile_start ("gde2_mixer_context_new");

                manager->priv->context = gde2_mixer_context_new ();
                ...
                gde2_mixer_context_open (manager->priv->context);

                gde2_settings_profile_end ("gde2_mixer_context_new");
        }
#endif
