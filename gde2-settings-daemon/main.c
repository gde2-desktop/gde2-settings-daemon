...
#define DEBUG_KEY             "gde2-settings-daemon"
#define DEBUG_SCHEMA          "org.gde2.debug"

#define GDE2_SESSION_DBUS_NAME      "org.gnome.SessionManager"
#define GDE2_SESSION_DBUS_OBJECT    "/org/gnome/SessionManager"
#define GDE2_SESSION_DBUS_INTERFACE "org.gnome.SessionManager"
#define GDE2_SESSION_PRIVATE_DBUS_INTERFACE "org.gnome.SessionManager.ClientPrivate"
...
static void
on_session_over (DBusGProxy *proxy, Gde2SettingsManager *manager)
{
        /* not used, see on_session_end instead */
}

static void
on_session_query_end (DBusGProxy *proxy, guint flags, Gde2SettingsManager *manager)
{
        GError *error = NULL;
        gboolean ret = FALSE;
...
static void
on_session_end (DBusGProxy *proxy, guint flags, Gde2SettingsManager *manager)
{
        GError *error = NULL;
        gboolean ret = FALSE;
...
static void
watch_for_term_signal (Gde2SettingsManager *manager)
{
        GIOChannel *channel;
...
static void
set_session_over_handler (DBusGConnection *bus, Gde2SettingsManager *manager)
{
        ...
        session_proxy =
                 dbus_g_proxy_new_for_name (bus,
                                            GDE2_SESSION_DBUS_NAME,
                                            GDE2_SESSION_DBUS_OBJECT,
                                            GDE2_SESSION_DBUS_INTERFACE);
        ...
        /* Register with gde2-session */
        startup_id = g_getenv ("DESKTOP_AUTOSTART_ID");
        if (startup_id != NULL && *startup_id != '\0') {
                res = dbus_g_proxy_call (session_proxy,
                                         "RegisterClient",
                                         &error,
                                         G_TYPE_STRING, "gde2-settings-daemon",
                ...
                if (!res) {
                        ...
                        private_proxy = dbus_g_proxy_new_for_name_owner (bus, GDE2_SESSION_DBUS_NAME,
                                                                         client_id, GDE2_SESSION_PRIVATE_DBUS_INTERFACE,
                                                                         &error);
...
main (int argc, char *argv[])
{
        ...
        bindtextdomain (GETTEXT_PACKAGE, GDE2_SETTINGS_LOCALEDIR);
        ...
        if (gde2_gsettings_schema_exists (DEBUG_SCHEMA))
        {
        ...
        gde2_settings_profile_start ("opening gtk display");
        if (! gtk_init_check (NULL, NULL)) {
                g_warning ("Unable to initialize GTK+");
                exit (EXIT_FAILURE);
        }
        gde2_settings_profile_end ("opening gtk display");
...
#ifdef HAVE_LIBNOTIFY
        notify_init ("gde2-settings-daemon");
#endif /* HAVE_LIBNOTIFY */

        gde2_settings_profile_start ("gde2_settings_manager_new");
        manager = gde2_settings_manager_new ();
        gde2_settings_profile_end ("gde2_settings_manager_new");
        if (manager == NULL) {
        ...
        if (g_getenv ("DBUS_STARTER_BUS_TYPE") == NULL) {
                error = NULL;
                res = gde2_settings_manager_start (manager, &error);
                if (! res) {
